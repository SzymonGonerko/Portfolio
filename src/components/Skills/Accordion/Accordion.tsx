...
export const Accordion = ({list}: props) => {
    ...
    const handleClick = (skill: string) => {
        const detailsSkills = document.querySelectorAll('.generalSkillLi') as unknown as HTMLCollection
        const skillsInfo = document.querySelector('.skillsInfo')
       
        const [...newArr] = detailsSkills
       
        const eachOther = newArr.filter((element) => !element.classList.contains(skill));
        ...
        const clickedItem = newArr.filter((element) => element.classList.contains(skill))[0]
 ...
 <nav className='mainSkillsNav'>
    ...
    {list.map((el : { title: string; subNav: any[]}, i : number ) => 
        <li key={i} onClick={() => handleClick(el.title)} className={`generalSkillLi ${el.title}`}>
        <p className='title'>{el.title === "OtherTools" ? "Other tools": el.title} <span className='arrow'><IoIosArrowDropdown /></span></p>
        <nav>
            <ul>
            {el.subNav.map((el: string, i: number) => <li key={i}>{el}</li>)}
            </ul>
        </nav>
        </li>
        )}
